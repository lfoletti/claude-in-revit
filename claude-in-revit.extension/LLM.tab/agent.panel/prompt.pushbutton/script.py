...
_STATIC_SYSTEM_PROMPT = (
    ...
    "  **Phase 2 (création BIM décomposée — user 2026-05-13)** : "
    "  pipeline en étapes auditables, **STOP après chaque étape** pour "
    "  permettre la validation visuelle user.\n"
    "  **Phase 2a — murs continus seuls** : (1) "
    "  `dwg_extract_wall_thicknesses_many` (info), (2) "
    "  **`dwg_create_continuous_walls_many`** avec `items=[{file_path, "
    "  level_ref, height_m}, …]`. Ce tool fusionne les fragments mur "
    "  via vote élévation (élévations linkées en Phase 1 servent de "
    "  source pour la continuité). Pas d'openings créés à ce stade. "
    "  (3) `views_open_3d` puis résumé. **STOP** : demande à l'user de "
    "  valider visuellement les murs avant de passer à Phase 2b.\n"
    "  **Phase 2b — ajout des openings** : (1) appel "
    "  `dwg_add_openings_to_walls_many` qui lit les openings des coupes, "
    "  vote l'orientation du mur hôte (EW vs NS) via les 4 élévations, "
    "  trouve le mur hôte parmi les Wall existants du KG, classify "
    "  porte/fenêtre via sill+height, crée via openings_create_many. "
    "  (2) `views_open_3d` puis résumé final.\n"
    "  **Fin** : `views_open_3d` pour validation visuelle, puis résumé "
    "  consolidé (Phase 1 + Phase 2 : fichiers, niveaux, types créés, "
    "  murs importés par niveau).\n"
