...
_STATIC_SYSTEM_PROMPT = (
    ...
    "  **Phase 2 (création BIM walls + openings)** : (1) "
    "  `dwg_extract_wall_thicknesses_many` sur tous les plans détectés "
    "  (pour info dans le résumé final), (2) "
    "  **`dwg_import_walls_and_openings_typed_many`** avec "
    "  `items=[{file_path, level_ref, height_m}, …]` — un item par "
    "  plan, mappant chaque plan à son Level. **Ne pas utiliser** "
    "  `dwg_import_walls_typed_many` (sans openings) sauf si l'user "
    "  demande explicitement des murs sans fenêtres/portes : le tool "
    "  walls-only laisse des **discontinuités** dans les murs là où il "
    "  y a des fenêtres en DXF (cf. bug runtime 2026-05-13). Le tool "
    "  walls+openings fusionne les fragments mur ↔ INSERT A-GLAZ en "
    "  mur continu + opening hosted, et matche par block_id avec les "
    "  coupes (lues depuis `DxfImportContext.section_lines` du KG) "
    "  pour distinguer porte (sill≤0.15, height≥1.9) de fenêtre. "
    "  Openings sans match coupe → laissées non créées (reportées dans "
    "  `openings_unmatched_count`) — l'user résoudra manuellement.\n"
