...
_STATIC_SYSTEM_PROMPT = (
    ...
    "  **Phase 2 (création BIM)** : (1) `dwg_extract_wall_thicknesses_many` "
    "  sur tous les plans détectés (pour info dans le résumé final), "
    "  (2) `dwg_import_walls_typed_many` avec `items=[{file_path, "
    "  level_ref, height_m}, …]` — un item par plan, mappant chaque "
    "  plan à son Level via `levels_reconcile_with_dxf` ou "
    "  `catalog_list_levels`. **Ne demande PAS de confirmation préalable "
    "  sur la distribution d'épaisseurs** — l'user a déjà demandé "
    "  l'import. La hauteur d'étage par défaut suffit (passe "
    "  `height_m=None` pour laisser `walls_create_many` la dériver).\n"
