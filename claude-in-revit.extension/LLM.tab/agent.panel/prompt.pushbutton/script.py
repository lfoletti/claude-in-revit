...
_STATIC_SYSTEM_PROMPT = (
    ...
    "  `views_link_cad`.\n"
    "- **IMPORT PROJET — STOP après Phase 1 (setup uniquement).** "
    "  Pour les prompts d'import projet (« importe ce projet », "
    "  « inspecte ce dossier de DXF », etc.), tu DOIS t'arrêter "
    "  après avoir : (a) inspecté les DXF, (b) reconcilié les "
    "  niveaux, (c) identifié les traits de coupe, (d) créé les "
    "  vues coupes Revit, (e) linké les DXF dans les vues — **TOUS "
    "  les DXF**, plan inclus. Le plan DXF se linke dans la vue "
    "  Plan d'étage du Niveau 0 (récupère `floor_plan_view_revit_id` "
    "  via `catalog_list_levels` qui l'expose quand appelé en Revit). "
    "  Chaque coupe DXF se linke dans sa ViewSection nouvellement "
    "  créée. **Chaque élévation DXF** (`kind='elevation'` avec "
    "  `direction` ∈ {Est, Nord, Sud, Ouest}) se linke dans la vue "
    "  élévation Revit correspondante — utilise "
    "  `catalog_list_elevation_views` pour trouver les `revit_id` "
    "  des 4 vues par défaut (N/S/E/W) et match par `direction`. "
    "  **Use les bulk variants `_many`** pour économiser les tokens : "
    "  `views_create_section_many` (toutes les coupes en 1 appel), "
    "  `views_link_cad_many` (tous les DXF en 1 appel), "
    "  `dxf_context_register_section_line_many`, "
    "  `dxf_context_register_linked_view_many`. Pour 8 DXF, c'est "
    "  ~14 appels → 2 appels = économie ~1200 tokens + 1 round-trip "
    "  API au lieu de 8. "
    "  **NE CRÉE PAS DE MURS, OUVERTURES, SOLS ou autres "
    "  éléments BIM** — pas même si l'inspection contient les données "
    "  nécessaires. C'est la Phase 2, distincte. **À la fin de Phase 1, "
    "  appelle `views_open_3d`** pour activer la vue 3D par défaut — "
    "  l'user verra immédiatement le résultat (plan + coupes linkées) "
    "  et pourra valider la cohérence du géo-référencement avant qu'on "
    "  commit le modèle. Puis rends à l'utilisateur un résumé de ce "
    "  qui a été fait + propose qu'il prompte à nouveau pour Phase 2 "
    "  (« crée les murs depuis le plan »).\n\n"
