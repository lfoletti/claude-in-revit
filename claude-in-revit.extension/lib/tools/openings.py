...
def _validate_delete_item(
    ...
    """Preflight one item from `openings_delete_many` : `{llm_id}` ou string."""
...
@tool(name="openings_delete_many", tier=1)
def delete_many(
    kg: ProjectKG,
    doc: Any,
    items: List[Any],
) -> Dict[str, Any]:
    """Supprime N portes/fenêtres en une seule Tx Revit + KG.

    Tolérant aux ElementId périmés (orphelins KG après suppression hors
    pipeline) : on soft-delete le KG sans crasher, et on signale
    `revit_already_gone` dans le payload.

    Concepts: porte, fenêtre, suppression, bulk, batch, masse, delete
    Phrases: "supprime toutes ces fenêtres", "delete openings",
             "vire ces portes", "remove windows"
    Similar: openings_delete, walls_delete_many
    ...
    with rp.transaction(doc, "openings.delete_many"):
        for nid in llm_ids:
            eid_raw = kg.get_revit_id(nid)
            if eid_raw is None:
                kg.soft_delete(nid)
